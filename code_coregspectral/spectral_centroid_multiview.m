function [F,P,R,nmi,avgent,AR,acc,prty] = spectral_centroid_multiview(X,num_views,numClust,sigma,lambda,truth,numiter)
% INPUT:
% OUTPUT:
...
%         fprintf('computing kernel for X(%d)\n',i);
    ...
    for j=1:kmeans_avg_iter
        ...
        [~,accj(j)] = myAccuracy(truth',C');
        prtyj(j) = purity(truth,C);
    end
