function [V,E,F,P,R,nmi,avgent,AR,C,acc,prty] = baseline_spectral_onRW(Prob,numClust,truth,projev)
...
%     [p,~]=eigs(Prob',1);
%     p=p/sum(p);
%     Prob=(diag(p)*Prob+Prob'*diag(p))/2;
    ...
    for i=1:10
        ...
        [~,acc(i)] = myAccuracy(truth',C');
        prty(i) = purity(truth,C);
    end
